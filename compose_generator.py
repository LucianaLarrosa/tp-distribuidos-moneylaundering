...
def _client(i, n_gateways):
    gateway_id = (i - 1) % n_gateways + 1
    return {
        "build": {"context": ".", "dockerfile": "src/client/Dockerfile"},
        "container_name": f"client_{i}",
        "depends_on": {f"gateway_{gateway_id}": {"condition": "service_started"}},
        "volumes": [
            "${DATASET_DIR:-./data}:/data:ro",
            "${OUTPUT_DIR:-./output}:/output",
        ],
        "environment": {
            "SERVER_HOST": f"gateway_{gateway_id}",
            "SERVER_PORT": "5000",
...
def build_compose(
    ...
    for i in range(1, clients + 1):
        services[f"client_{i}"] = _client(i, gateways)
