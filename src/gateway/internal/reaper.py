...
class Reaper:
        ...
        try:
            while not self._stop.wait(self._config.reaper_interval):
                try:
                    ...
                    for client_id in self._registry.dead_clients(
                        self._config.reaper_timeout
                    ):
                        logging.info("[reaper] reaping dead client %s", client_id)
                        router.forward_cleanup_eof(client_id)
                        self._delete_results_queue(client_id)
    ...
    def _delete_results_queue(self, client_id):
        queue_name = f"{self._config.results_queue_prefix}.{client_id}"
        results = MessageMiddlewareExchangeDirectRabbitMQ(
            self._config.rabbitmq_host,
            self._config.query_results_exchange,
            [client_id],
            queue_name=queue_name,
        )
        try:
            results.delete_queue()
            logging.info("[reaper] deleted results queue %s", queue_name)
        finally:
            results.close()
